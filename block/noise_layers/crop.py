...
class Cropout(nn.Module):
	...
	def forward(self, image_and_cover):
		org_image, cover_image = image_and_cover

		h_start, h_end, w_start, w_end = get_random_rectangle_inside(org_image.shape, self.height_ratio,
																	 self.width_ratio)
		output = cover_image.clone()
		output[:, :, h_start: h_end, w_start: w_end] = org_image[:, :, h_start: h_end, w_start: w_end]
		return output
